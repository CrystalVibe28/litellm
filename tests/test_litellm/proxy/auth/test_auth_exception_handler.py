...
@pytest.mark.asyncio
async def test_handle_authentication_error_logs_resolved_user_context():
    handler = UserAPIKeyAuthExceptionHandler()
    mock_request = MagicMock()
    mock_request_data = {"model": "gpt-4o"}
    test_route = "/chat/completions"
    resolved_token = UserAPIKeyAuth(
        api_key="hashed-key",
    ...
    with (
        ...
        with pytest.raises(ProxyException):
            await handler._handle_authentication_error(
                ...
                None,
                "raw-key",
                resolved_identity=resolved_token,
            )

        mock_post_call_failure_hook.assert_awaited_once()
        user_api_key_dict = mock_post_call_failure_hook.call_args.kwargs["user_api_key_dict"]
        assert user_api_key_dict is not resolved_token
        assert user_api_key_dict.user_id == "openwebui-user-a"
        assert user_api_key_dict.end_user_id == "openwebui-user-name"
        assert user_api_key_dict.request_route == test_route
        assert resolved_token.request_route is None
