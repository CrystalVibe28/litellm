...
@pytest.mark.asyncio
async def test_enqueue_daily_spend_updates_isolates_per_entity_failures():
    """
    A failure on one entity's add_update must not block the remaining entity
    rows for the same request. Matches the prior per-entity try/except
    isolation behavior before _enqueue_daily_spend_updates was introduced.
    """
    writer = DBSpendUpdateWriter()
    mock_prisma = MagicMock()
    mock_prisma.get_request_status = MagicMock(return_value="success")
    ...
    # First entity blows up; every subsequent entity must still be enqueued.
    writer.daily_spend_update_queue.add_update = AsyncMock(
        side_effect=RuntimeError("user enqueue boom")
    )
    writer.daily_team_spend_update_queue.add_update = AsyncMock()
    writer.daily_org_spend_update_queue.add_update = AsyncMock()
    writer.daily_end_user_spend_update_queue.add_update = AsyncMock()
    ...
    writer.daily_end_user_spend_update_queue.add_update.assert_awaited_once()
    writer.daily_agent_spend_update_queue.add_update.assert_awaited_once()
    # Two tags, both should have been attempted.
    assert writer.daily_tag_spend_update_queue.add_update.await_count == 2


@pytest.mark.asyncio
async def test_enqueue_daily_spend_updates_isolates_per_tag_failures():
    """
    A failure enqueueing one tag's daily row must not block enqueueing the
    other tags in the same request.
    """
    writer = DBSpendUpdateWriter()
    mock_prisma = MagicMock()
    mock_prisma.get_request_status = MagicMock(return_value="success")
    payload = {
        "request_id": "req-tag-isolation",
        "request_tags": ["tag-a", "tag-b", "tag-c"],
        "startTime": "2024-01-01T12:00:00",
        "api_key": "test-key",
        "model": "gpt-4",
        "custom_llm_provider": "openai",
    ...
    async def add_update(update):
        (key,) = update.keys()
        call_log.append(key)
        if "tag-b" in key:
            raise RuntimeError("tag-b enqueue boom")

    writer.daily_tag_spend_update_queue.add_update = AsyncMock(side_effect=add_update)

    await writer._enqueue_daily_spend_updates(
        payload=payload, org_id=None, prisma_client=mock_prisma
    )

    assert writer.daily_tag_spend_update_queue.add_update.await_count == 3
    assert any("tag-a" in k for k in call_log)
    assert any("tag-b" in k for k in call_log)
    assert any("tag-c" in k for k in call_log)


@pytest.mark.asyncio
async def test_enqueue_daily_spend_updates_skips_malformed_request_tags_json():
    """
    request_tags arriving as an invalid JSON string must be skipped with a
    warning, not raise out of _enqueue_daily_spend_updates.
    """
    writer = DBSpendUpdateWriter()
    mock_prisma = MagicMock()
    mock_prisma.get_request_status = MagicMock(return_value="success")
    payload = {
        "request_id": "req-bad-tags",
        "user": "test-user",
        "request_tags": "{not-valid-json",
        "startTime": "2024-01-01T12:00:00",
        "api_key": "test-key",
        "model": "gpt-4",
        "custom_llm_provider": "openai",
...
def test_get_base_daily_spend_transaction_handles_malformed_metadata(metadata_value):
    """
    A malformed `metadata` field must not raise. The base transaction should
    still resolve - cache_*_tokens default to 0 - so the per-request daily
    spend writes are not silently dropped.
    """
    writer = DBSpendUpdateWriter()
    mock_prisma = MagicMock()
    mock_prisma.get_request_status = MagicMock(return_value="success")
    ...
    result = writer._get_base_daily_spend_transaction(
        payload=payload, prisma_client=mock_prisma
    )
...
def test_get_base_daily_spend_transaction_accepts_metadata_dict():
    """`metadata` may arrive as a dict (not yet JSON-serialized); honor it."""
    writer = DBSpendUpdateWriter()
    mock_prisma = MagicMock()
    mock_prisma.get_request_status = MagicMock(return_value="success")
    ...
    result = writer._get_base_daily_spend_transaction(
        payload={
            ...
            "completion_tokens": 5,
            "spend": 0.1,
            "metadata": {"usage_object": {"cache_read_input_tokens": 7}},
        },
        prisma_client=mock_prisma,
    )
